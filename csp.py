...
class CSP(search.Problem):
    ...
    A CSP is specified by the following inputs:
        ...
        neighbors   A dict of {var:[var,...]} that for each variable lists
                ...
                tuple_constraints A tuple (f,...) that contains all the functions 
                                        (constraints) f(A, a, B, b) that returns true if neighbors
                                A, B satisfy the constraint when they have values A=a, B=b
                var_to_cons A dict of {var:[f,...]} that for each variable lists the 
                functions(constraints) that the variable participates in
    ...
    def __init__(self, variables, domains, neighbors, tuple_constraints, var_to_cons):
    ...
    def conflicted_vars(self, current):
        ...
        # This function is for general use

    def constraints(self, A, a, B, b):
        """Return a tuple that contains whether there is a conflict between the 
        values a and b and also the constraint that caused it"""
        self.constraints_count += 1
        for conA in self.var_to_cons[A]:
            for conB in self.var_to_cons[B]:
                if conA == conB:
                    if not conA(A, a, B, b):
                        return False, conA
                    break
...
def AC3(csp, queue=None, removals=None, arc_heuristic=dom_j_up):
    ...
    while queue:
        (Xi, Xj) = queue.pop()
        revised, checks, constraint_applied = revise(
            csp, Xi, Xj, removals, checks)
        if revised:
            if not csp.curr_domains[Xi]:
                # increase weight by 1, when domain wipeout occurs
                csp.weights[constraint_applied] += 1
...
def revise(csp, Xi, Xj, removals, checks=0):
    ...
    for x in csp.curr_domains[Xi][:]:
        ...
        for y in csp.curr_domains[Xj]:
            check, constraint_applied = csp.constraints(Xi, x, Xj, y)
            if check:
                conflict = False
            checks += 1
            if not conflict:
...
def dom_wdeg(assignment, csp):
    return argmin_random_tie([v for v in csp.variables if v not in assignment],
                             key=lambda var: dom_wdeg_ratio(csp, var, assignment))
...
# dom/wdeg ratio calculator

def dom_wdeg_ratio(csp, var, assignment):
    if csp.curr_domains:
        sum = 0
        # Constraints that apply to the variable
        for con in csp.var_to_cons[var]:
            sum += csp.weights[con]  # Sum of weights of these constraints

        if sum != 0:
            return len(csp.curr_domains[var])/sum  # ratio
        else:
    ...
    for B in csp.neighbors[var]:
        if B not in assignment:
            for b in csp.curr_domains[B][:]:
                check, constraint_applied = csp.constraints(var, value, B, b)
                if not check:
